...
ReportModule::ReportModule(int n, book b[]){
	num = n;
	for (int x = 0; x < num; x++)
		isbn[x] = b[x].getISBN();
	for (int x = 0; x < num; x++)
		title[x] = b[x].getTitle();
	for (int x = 0; x < num; x++)
		author[x] = b[x].getAuthor();
	for (int x = 0; x < num; x++)
		publisher[x] = b[x].getPublisher();
	for (int x = 0; x < num; x++)
		dateAdded[x] = b[x].getDateAdded();
	for (int x = 0; x < num; x++)
		qtyOnHand[x] = b[x].getOnHand();
	for (int x = 0; x < num; x++)
		wholesaleCost[x] = b[x].getWholesaleCost();
	for (int x = 0; x < num; x++)
		retailPrice[x] = b[x].getRetailCost();
}
